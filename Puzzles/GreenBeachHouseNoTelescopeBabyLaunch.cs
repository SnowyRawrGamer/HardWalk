...
// Green Tower beach house: remove the telescope and relocate/launch the solved-puzzle baby over water.
...
internal static class GreenBeachHouseNoTelescopeBabyLaunch
{
    ...
    private static void CompletedPostfix(Rigidbody baby, Transform beachHouseRoot)
    {
        ...
        var launch = (Vector3.forward * 0.7f + Vector3.up * 1.1f).normalized;
        baby.AddForce(launch * 24f, ForceMode.Impulse);
    ...
    private static void SpawnRemoteRock(Transform root)
    {
        ...
        rock.transform.position = RemoteRockPosition;
        rock.transform.localScale = new Vector3(4f, 0.5f, 4f);
    }
}
